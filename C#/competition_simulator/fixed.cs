    ...
    class Player{
        ...
        public void set_status(string mode, int new_hp = 0, int new_dmg = 0){
            if (string.IsNullOrEmpty(mode)){
                throw new NullReferenceException("mode must given");
            }
            ...
            if (!mode_valid){
                throw new Exception("mode is not valid, mode should be an element in Player.stats.");
            }
        ...
        public void print_status(){
            Console.Write("- {0:s}:\t", this.name);
            Console.Write("{0:d}: {1:d}\t", "hp", this.hp);
            Console.Write("{0:d}: {1:d}\t\n", "damage", this.attack_dmg);
        }
    }
